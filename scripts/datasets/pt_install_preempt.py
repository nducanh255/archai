...
def untar_dataset(conf_name:str, pt_data_dir:str, conf_dataset:Config, dataroot:str)->None:
    ...
    return_code = exec_shell_command_async(command)

    if return_code == 0:
        print(f'dataset copied from {tar_filepath} to {local_dataroot} sucessfully')
    else:
        raise Exception(f'Copying dataset did not complete successfully with error code {return_code}')

def exec_shell_command_async(command:str, print_command_start=True, print_command_end=True)->subprocess.CompletedProcess:
    ...
    # Dummy commands to utilize GPUs while copying large datasets so that job doesn't get killed for idle GPUs
    bs = 4096
    while p.poll() is None:
        try:
            inp = torch.randn(bs,3,224,224).cuda()
            out = model(inp)
        except Exception as e:
            bs = bs // 2

    if print_command_end:
        print(f'[{datetime.now()}] returncode={p.returncode} Finished: {command}')
