...
from archai.common.metrics_ssl import EpochMetricsSimClr, MetricsSimClr
...
class SearchResult:
    def __init__(self, model_desc:Optional[ModelDesc],
                 search_metrics:Optional[MetricsSimClr],
                 train_metrics:Optional[MetricsSimClr]) -> None:
...
class SearcherSimClr(EnforceOverrides):
    ...
    def clean_log_result(self, conf_search:Config, search_result:SearchResult)->None:
        ...
        if search_result.search_metrics is not None:
            logger.info({'search_loss_val':
                search_result.search_metrics.best_val_loss()})
        if search_result.train_metrics is not None:
            logger.info({'train_loss_val':
                search_result.train_metrics.best_val_loss()})
    ...
    def search_model_desc(self, conf_search:Config, model_desc:ModelDesc,
                          trainer_class:TArchTrainer, finalizers:Finalizers)\
                              ->Tuple[ModelDesc, Optional[MetricsSimClr]]:
    ...
    def train_model_desc(self, model_desc:ModelDesc, conf_train:Config)\
        ...
        model = ModelSimClr(model_desc, conf_projection['hidden_dim'], conf_projection['out_features'], droppath=drop_path_prob>0.0, affine=True)

        # get data
        data_loaders= self.get_data(conf_loader)

        trainer = TrainerSimClr(conf_trainer, model, checkpoint=None)
