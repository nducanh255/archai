...
class Cars196Provider(DatasetProvider):
    ...
    def get_datasets(self, load_train:bool, load_test:bool,
        ...
        if load_train:
            trainpath = os.path.join(self._dataroot, 'cars196', 'train')
            trainset = torchvision.datasets.ImageFolder(trainpath, transform=transform_train)
        if load_test:
            testpath = os.path.join(self._dataroot, 'cars196', 'test')
    ...
    def get_transforms(self)->tuple:
        # MEAN, STD computed for imagenet
...
register_dataset_provider('cars196', Cars196Provider)