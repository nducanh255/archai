...
class INat21Provider(DatasetProvider):
    ...
    def get_datasets(self, load_train:bool, load_test:bool,
        ...
        if load_train:
            trainset = datasets.ImageFolder(root=os.path.join(self._dataroot, 'inat21', 'train'),
                transform=transform_train)
            # compatibility with older PyTorch
            if not hasattr(trainset, 'targets'):
                trainset.targets = [lb for _, lb in trainset.samples]
        if load_test:
            testset = datasets.ImageFolder(root=os.path.join(self._dataroot, 'inat21', 'val'),
...
register_dataset_provider('inat21', INat21Provider)
