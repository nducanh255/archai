...
def get_prev_config(keys=None):
    dir = utils.full_path(common.default_resultroot())
    files_list = os.listdir(dir)
    for f in files_list:
        if f=='config.yaml':
            config_file_path = os.path.join(dir, f)
            with open(config_file_path) as f:
                config = yaml.load(f, Loader=yaml.FullLoader)
            
            if keys is not None:
                curr_keys = list(config.keys())
                for k in curr_keys:
                    if not k in keys:
                        del config[k]
                
            # # delete unwanted configs
            # del config['ppl_threshold']
            # del config['work_dir']
            # del config['data']
            return config

    assert False, 'No config file found in the mapped (previous) AMLT job'
...
def parse_args():
    ...
    # load configuration from a previous job 
    previous_config = get_prev_config()
    config['fp16'] = previous_config['fp16']
    ...
    general_keys = args_.__dict__.keys()
    ...
    args_, _ = parser.parse_known_args() 
    general_keys = list(general_keys)+['data']
    dataset_keys = np.setdiff1d(list(args_.__dict__.keys()), list(general_keys))
    ...
    model.add_argument('--proj_init_std', type=float, default=0.01,
                       help='Parameters initialized by N(0, init_std)')
    args_, _ = parser.parse_known_args() 
    model_keys = np.setdiff1d(list(args_.__dict__.keys()), list(general_keys)+list(dataset_keys))
    ...
    args_, _ = parser.parse_known_args() 
    opt_keys = np.setdiff1d(list(args_.__dict__.keys()), list(general_keys)+list(dataset_keys)+list(model_keys))
    ...
    args_, _ = parser.parse_known_args() 
    training_keys = np.setdiff1d(list(args_.__dict__.keys()), list(general_keys)+list(dataset_keys)+list(model_keys)+list(opt_keys))
    ...
    args_, _ = parser.parse_known_args() 
    val_keys = np.setdiff1d(list(args_.__dict__.keys()), list(general_keys)+list(dataset_keys)+list(model_keys)+list(opt_keys)+list(training_keys))

    fear = parser.add_argument_group('FEAR setup')
    fear.add_argument('--ppl_threshold', type=float,
                        help='Perplexity threshold for the FEAR checkpoint')
    fear.add_argument('--n_unfreeze', type=int, default=1,
                     help='Number of layers to unfreeze (starts from )')
    fear.add_argument('--granular_unfreeze', action='store_true',
                        help='Freezes individual Attn/FF layers rather than the entire decoder')
    args_, _ = parser.parse_known_args() 
    fear_keys = np.setdiff1d(list(args_.__dict__.keys()), list(general_keys)+list(dataset_keys)+list(model_keys)+list(opt_keys)+list(training_keys)+list(val_keys))
    ...
    args_, _ = parser.parse_known_args() 
    dist_keys = np.setdiff1d(list(args_.__dict__.keys()), list(general_keys)+list(dataset_keys)+list(model_keys)+list(opt_keys)+list(training_keys)+list(val_keys)+list(fear_keys))

    # only take useful info out of the previous config
    previous_config = get_prev_config(keys=np.concatenate((dataset_keys, model_keys, opt_keys)))
...
def freeze_layers(model, n_unfreeze=1, layerType=[AdaptiveEmbedding, DecoderLayer, RelLearnableDecoderLayer, \
                                                    RelPartialLearnableDecoderLayer, ProjectedAdaptiveLogSoftmax]):
    layers = get_list_of_layers(model, layerType=layerType)

    for param in model.parameters():
        param.requires_grad = False

    # start from the last layer and unfreeze
    unfreeze = 0
    for l in layers[::-1]:
        for p in l.parameters():
            p.requires_grad = True
        unfreeze += 1
        if unfreeze==n_unfreeze:
            break

    for name, param in model.named_parameters():
        if param.requires_grad:
            logging.info(f'{name} requires grad')
...
def load_fear_checkpoint(ppl_threshold):
    import re
    ...
    for f in files_list:
        if 'fear' in f:
            fname = f.replace('.pt', '')
            idx = re.search('threshold_', fname).span()[0]
            f_threshold = float(fname[idx:].split('_')[1])
            if f_threshold == ppl_threshold:
                path = os.path.join(dir, f)
                dst = f'cuda:{torch.cuda.current_device()}'
                logging.info(f'Loading fear checkpoint from {path}')
                checkpoint = torch.load(path, map_location=dst)
                break
    
    if checkpoint is None:
        logging.info(f'No fear checkpoint found with {ppl_threshold} ppl')
        sys.exit(1)
...
def main():
    ...
    # load from fear stage 1 checkpoint
    try:
        checkpoint = load_fear_checkpoint(args.ppl_threshold)
        ...
        logging.info(f'Could not load checkpoint from {args.restart}, Stopping')
    
    val_loss = evaluate(va_iter, model, args)
    val_loss = nv_distributed.all_reduce_item(val_loss, op='mean')
    curr_ppl = math.exp(val_loss)
    logging.info('ppl of loaded model: {:.2f}'.format(curr_ppl))

    # freeze layers for fear stage 2
    if args.granular_unfreeze:
        layerType = [PositionwiseFF, MultiHeadAttn, RelMultiHeadAttn, RelPartialLearnableMultiHeadAttn, RelLearnableMultiHeadAttn, AdaptiveEmbedding, ProjectedAdaptiveLogSoftmax]
    else:
        layerType = [DecoderLayer, RelLearnableDecoderLayer, RelPartialLearnableDecoderLayer, AdaptiveEmbedding, ProjectedAdaptiveLogSoftmax]
    
    freeze_layers(model, n_unfreeze=args.n_unfreeze, layerType=layerType)
